...
import type { ApiInspectReport, ApiInspectReportEntry, User } from "../models";
import type { AssociationIds } from "type_aliases";

export default class ReportApi {
  static createInspectReport({ projectId }: AssociationIds,
                             report: ApiInspectReport,
                             user: User): Promise<ApiInspectReport> {
  ...
  static listInspectReportEntries({ projectId, inspectReportId }: AssociationIds,
                                  user: User): Promise<ApiInspectReportEntry[]> {
  ...
  static createInspectReportEntries({ projectId, inspectReportId }: AssociationIds,
                                    reportEntries: ApiInspectReportEntry[],
                                    user: User): Promise<ApiInspectReportEntry[]> {
    let url = `${Http.baseUrl()}/projects/${projectId}/reports/${inspectReportId}/inspect-entries`;
    return Http.post(url, user, reportEntries) as unknown as Promise<ApiInspectReportEntry[]>;
  }

  static updateInspectReportEntry({ projectId, inspectReportId, inspectReportEntryId }: AssociationIds,
                                  reportEntry,
                                  user): Promise<ApiInspectReportEntry> {
  ...
  static deleteInspectReportEntry({ projectId, inspectReportId, inspectReportEntryId }: AssociationIds,
