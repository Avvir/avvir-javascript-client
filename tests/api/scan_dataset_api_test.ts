...
0 0 0 5.7777`,
  ...
  describe("#clearVerified", () => {
    beforeEach(() => {
      fetchMock.post(`${Http.baseUrl()}/projects/some-project-id/floors/some-floor-id/scan-datasets/some-scan-dataset-id/clear-verified`, 200);
    });

    it("makes a request to the gateway", () => {
      ScanDatasetApi.clearVerified({
        projectId: "some-project-id",
        floorId: "some-floor-id",
        scanDatasetId: "some-scan-dataset-id"
      }, {
      ...
      expect(fetchMock.lastUrl()).to.eq(`${Http.baseUrl()}/projects/some-project-id/floors/some-floor-id/scan-datasets/some-scan-dataset-id/clear-verified`);
    });
