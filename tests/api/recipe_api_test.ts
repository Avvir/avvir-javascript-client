...
describe("RecipeApi", () => {
  ...
  describe("::createProjectTradeRecipe", () => {
    beforeEach(() => {
      fetchMock.post(`${Http.baseUrl()}/projects/some-project-id/trade-recipes`, 200);
    });

    it("makes a request to the gateway api", () => {
      RecipeApi.createProjectTradeRecipe({ projectId: "some-project-id" },
        new ApiRecipe({
          name: "some recipe name",
          isEnabled: true,
          modeledCode: "1234"
      ...
      expect(fetchCall[0]).to.eq(`${Http.baseUrl()}/projects/some-project-id/trade-recipes`);
      expect(JSON.parse(fetchCall[1].body as string)).to.deep.eq({
        "isEnabled": true,
        "modeledCode": "1234",
        "name": "some recipe name",
  ...
  describe("::listProjectTradeRecipes", () => {
    beforeEach(() => {
      fetchMock.get(`${Http.baseUrl()}/projects/some-project-id/trade-recipes`, 200);
    });

    it("makes a call to the endpoint", () => {
      RecipeApi.listProjectTradeRecipes({ projectId: "some-project-id" }, user);
    ...
    it("sends the request with an Authorization header", () => {
      RecipeApi.listProjectTradeRecipes({ projectId: "some-project-id" }, {
        authType: UserAuthType.GATEWAY_JWT,
        gatewayUser: { idToken: "some-firebase.id.token", role: UserRole.USER }
      });
  ...
  describe("::updateProjectTradeRecipe", () => {
    beforeEach(() => {
      fetchMock.put(`${Http.baseUrl()}/projects/some-project-id/trade-recipes/7`, 200);
    });

    it("makes a request to the gateway api", () => {
      RecipeApi.updateProjectTradeRecipe({ projectId: "some-project-id" },
        new ApiRecipe({
          id: 7,
          name: "some updated name",
          isEnabled: true,
      ...
      expect(fetchCall[0]).to.eq(`${Http.baseUrl()}/projects/some-project-id/trade-recipes/7`);
      expect(JSON.parse(fetchCall[1].body as string)).to.deep.eq({
        id: 7,
        isEnabled: true,
        modeledCode: "1234",
  ...
  describe("::deleteProjectTradeRecipe", () => {
    beforeEach(() => {
      fetchMock.delete(`${Http.baseUrl()}/projects/some-project-id/trade-recipes/7`, 200);
    });

    it("makes a request to the gateway api", () => {
      RecipeApi.deleteProjectTradeRecipe({ projectId: "some-project-id" }, 7, {
        authType: "GATEWAY_JWT",
        gatewayUser: { idToken: "some-firebase.id.token" }
      } as User);
      const fetchCall = fetchMock.lastCall();
  ...
  describe("::deleteProjectTradeRecipeStep", () => {
    beforeEach(() => {
      fetchMock.delete(`${Http.baseUrl()}/projects/some-project-id/trade-recipes/7/steps/9`, 200);
    });

    it("makes a request to the gateway api", () => {
      RecipeApi.deleteProjectTradeRecipeStep({ projectId: "some-project-id" }, 7, 9, {
        authType: "GATEWAY_JWT",
        gatewayUser: { idToken: "some-firebase.id.token" }
      } as User);
      const fetchCall = fetchMock.lastCall();

      expect(fetchCall[0]).to.eq(`${Http.baseUrl()}/projects/some-project-id/trade-recipes/7/steps/9`);
      expect(fetchMock.lastOptions().headers.Accept).to.eq("application/json");
    });
